...
- Số điện thoại liên hệ: 0976100286

QUY TẮC:
- CHỈ trả lời dựa trên TÀI LIỆU được cung cấp trong câu hỏi. TUYỆT ĐỐI KHÔNG bịa đặt.
- Thêm chút nhí nhỏm tạo sự thoải mái cho phụ huynh khi tương tác.
- Chi phí ăn bán trú là 17.000 đồng/ngày/trẻ.
- Nếu không có tài liệu kèm theo, đó là câu chào hỏi/cảm ơn/tạm biệt => đáp lại thân thiện.
- Nếu có tài liệu nhưng không đủ thông tin => hướng dẫn liên hệ SĐT 0976100286.
...
// Hàm cắt bỏ lời chào ở đầu câu trả lời của AI
function removeGreeting(text) {
    // Các pattern chào thường gặp ở đầu câu
    const greetingPatterns = [
        /^(Dạ\s+)?(Chào|Xin chào|Hello|Hi)\s+(bố\/?mẹ|phụ huynh|quý phụ huynh|anh|chị|bạn)[^\n]*[!.?\s🥰👋😊💖✨]*\n*/i,
        /^(Dạ\s+)?Chào\s+(bố\/?mẹ|phụ huynh)[^\n]*\n*/i,
    ];
    let result = text.trimStart();
    for (const pattern of greetingPatterns) {
        result = result.replace(pattern, '').trimStart();
    }
    // Nếu sau khi cắt còn trống thì trả về text gốc
...
export async function askGemini(context, question, history = []) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            const text = removeGreeting(rawText);
            if (rawText !== text) console.log('[GEMINI] 🔧 Đã cắt lời chào ở đầu câu trả lời');
    ...
    return "Dạ hiện tại hệ thống AI đang gặp chút sự cố nhỏ 😅. Bố/mẹ có thể liên hệ trực tiếp với nhà trường qua fanpage hoặc số điện thoại 0976100286 để được hỗ trợ nhanh nhất nhé! Cảm ơn bố mẹ rất nhiều ạ 💖";
}
