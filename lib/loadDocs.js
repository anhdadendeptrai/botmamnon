...
export async function loadDocs() {
    ...
    if (fs.existsSync(huongDanDir)) {
        ...
        // Process .pdf files
        const pdfFiles = fs.readdirSync(huongDanDir).filter(f => f.endsWith('.pdf'));
        const pdfDocsHuongDan = await Promise.all(
            pdfFiles.map(async (filename) => {
                const filePath = path.join(huongDanDir, filename);
                const buffer = fs.readFileSync(filePath);
                try {
                    const pdfData = await pdfParse(buffer);
                    const title = filename.replace('.pdf', '').replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
                    return { filename, title, content: pdfData.text.trim() };
                } catch (err) {
                    console.error(`Lỗi đọc file ${filename}:`, err);
                    return null;
                }
        ...
        docs.push(...pdfDocsHuongDan.filter(Boolean));
    }
